...
export default function DMPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("dm-major-enemy-sound-sync")
      .on(
        "postgres_changes",
  ...
  async function uploadCombatBackground(file: File) {
    const fileExt = file.name.split(".").pop()
    const filePath = `combat/${Date.now()}.${fileExt}`
  ...
  async function duplicateEnemy(enemy: Character) {
    ...
    if (error) {
      console.log("DUPLICATE ENEMY ERROR:", error.message)
      return
    }
  ...
  async function removeEnemy(id: string) {
    ...
    if (error) {
      console.log("REMOVE ENEMY ERROR:", error.message)
      return
    }
  ...
  if (error) {
    console.log("SAVE MAJOR ENEMY ERROR:", error.message)
    return
  }
  ...
  async function addSavedMajorEnemyToEncounter(template: SavedMajorEnemy) {
    ...
    if (error) {
      console.log("LOAD SAVED MAJOR ENEMY ERROR:", error.message)
      return
    }
  ...
  async function deleteSavedNpc(id: string) {
    const { error } = await supabase.from("saved_npcs").delete().eq("id", id)
    if (error) console.log("DELETE SAVED ENEMY ERROR:", error.message)
    await loadAll()
  }
  ...
  async function deleteSavedMajorEnemy(id: string) {
    ...
    if (error) console.log("DELETE SAVED MAJOR ENEMY ERROR:", error.message)
    await loadAll()
  }
    ...
    <main className="min-h-screen bg-zinc-950 text-white p-8">
      <div className="flex justify-between items-start mb-10">
            ...
            <div className="absolute left-0 top-16 w-[34rem] max-w-[90vw] bg-zinc-900 border border-yellow-700 rounded-2xl shadow-2xl z-40 overflow-hidden">
              ...
              {showBackgroundControls && (
                <div className="p-5 border-b border-zinc-700">
                  ...
                  {encounter?.background_url && (
                    <img
                      src={encounter.background_url}
                      alt="Combat Background"
                      className="mt-5 rounded-xl border border-zinc-700 max-h-52 object-cover"
                    />
                  )}
                </div>
              )}
              ...
              {showSavedNpcs && (
                <div className="p-5 border-t border-zinc-700 max-h-[28rem] overflow-y-auto">
                  {savedNpcs.length === 0 ? (
                    <p className="text-zinc-400">No saved enemies yet.</p>
                  ) : (
                    <div className="space-y-3">
                      {savedNpcs.map((npc) => (
                        ...
                        >
                          ...
                          <p className="text-zinc-400">{npc.class_name}</p>
                          <p className="mt-2">HP: {npc.max_hp}</p>
                          <p>AC: {npc.ac}</p>
              ...
              {showSavedMajorEnemies && (
                <div className="p-5 border-t border-zinc-700 max-h-[28rem] overflow-y-auto">
                  {savedMajorEnemies.length === 0 ? (
                    <p className="text-zinc-400">No saved major enemies yet.</p>
                  ) : (
                    <div className="space-y-3">
                      {savedMajorEnemies.map((enemy) => (
                        <div
                          key={enemy.id}
                          className="bg-zinc-800 border border-purple-700 rounded-xl p-4"
                        >
                          <h3 className="text-lg font-bold text-purple-400">
                            {enemy.name}
                          </h3>

                          <p className="text-zinc-400">{enemy.class_name}</p>
                          <p className="mt-2">HP: {enemy.max_hp}</p>
                          <p>AC: {enemy.ac}</p>
                          ...
                          <div className="flex gap-2 mt-4">
                            <button
                              onClick={() =>
                                addSavedMajorEnemyToEncounter(enemy)
                              }
        ...
        <div className="text-center">
          ...
          <p className="text-zinc-400 mt-2">Encounter Control Center</p>
        </div>
